...
class HREmployeeAdvance(Document):
	pass



@frappe.whitelist()
def get_employee_advance(payroll_month):
	...
	if isinstance(json_response,list):
		...
		for row in json_response :
			...
			doc_props = {
				...
				"hr_id":row.get("Trx_ser"),
				"employee":row.get("Staff_ID"),
				"advance_amount":row.get("TOTALvalue") or 0,
				"posting_date":row.get("Trx_date") or nowdate(),
			}
			employee = ""
			employee_name = ""
			...
			try :
				create_employee_advance(doc.name)
			except Exception as e :
				frappe.msgprint(f"{doc.name} Error => " + str(e),indicator='red')
				error_list.append(row.get("Trx_ser"))
				if doc :
					doc.delete()
...
@frappe.whitelist()
# def create_employee_advance(employee_no,advance_amount=0,posting_date=nowdate()):        
def create_employee_advance(hr_employee_advance):  
	doc =  frappe.get_doc("HR Employee Advance",hr_employee_advance)    
